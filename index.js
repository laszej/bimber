...
function randomLady(){
   ...
   let randomCardLady = ladies[random]

   const opis = document.createElement("div")
   opis.innerHTML = `${randomCardLady.name} | ${randomCardLady.age} | ${randomCardLady.place} `
   ...
   card.style.backgroundImage = `url("${randomCardLady.image}")`
   container.appendChild(card)
   yes.addEventListener("click", ()=>{container.removeChild(card); randomLady()})
   no.addEventListener("click", ()=>{container.removeChild(card); randomLady()})
